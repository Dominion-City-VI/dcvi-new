...
import { postDepartmentMember } from '@/requests/department';
...
export default function CreateMemberModal() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: postDepartmentMember,
  ...
  // }
   ...
   function onSubmit(values: TOnboardMemberSchema) {
    if (!userExtraInfo.departmentId) {
      throw new Error('CreateMemberModal: cannot submit without departmentId');
    }

    const payload: TOnboardMemberSchema = {
      ...values,
      isAssistantCellLeader: false,
      isConsideredLeader: false,
      isDcMember: true,
    ...
    mutate({ payload, departmentId: userExtraInfo.departmentId});
  }
  ...
  return (
    <XModal isOpen={isOpen.DEPARTMENT_ADD_MEMBER} closeModal={() => !isPending && toggleModals({})}>
