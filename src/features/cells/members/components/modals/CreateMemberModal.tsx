...
export default function CreateMemberModal() {
  ...
  const { data: deptData, status: deptStatus, isLoading: isDeptLoading } = useFetchDepartments();
   ...
   console.log("Zonal ID",  cellModal?.zonalId)
    console.log("Vell ID",  cellModal?.cellId)

  const form = useForm({
    mode: 'onSubmit',
    defaultValues: {
      zoneId: cellModal?.zonalId || userExtraInfo.zonalId ||'',
      cellId: cellModal?.cellId || userExtraInfo.cellId || '',
      firstName: '',
      lastName: '',
      emailAddress: '',
      phoneNumber: '',
