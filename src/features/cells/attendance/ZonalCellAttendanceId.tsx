...
import { Route } from '@/routes/_authenticated/cell/attendance/$cellAttendanceCellId';

const ZonalCellAttendanceId = () => {
  const { cellAttendanceCellId } = Route.useParams();
  ...
  const { data, isLoading } = useFetchAttendance({
    Identifier: cellAttendanceCellId,
    ...attendanceQuery
  });
...
export default observer(ZonalCellAttendanceId);